    ...
    def __init__(self, pageName):
        if not (pageName[:3].find("map") == -1):
            ...
            if (str(mapNumber) in [str(i) for i in range(0, 99)]):
                if not (pageName.find("dungeon_ex") == -1):
                    if not (pageName.find("ex1_clear") == -1):
                        ...
                        self.setName("Chapter " + mapNumber + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"] + " [ Ending 1 ]")
                    elif not (pageName.find("ex2_clear") == -1):
                        self.setOrder(int(mapNumber)*1000+994)
                        self.setDirectory(pageName)
                        self.setName("Chapter " + mapNumber + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"] + " [ Ending 2 ]")
                    elif not (pageName.find("ex_clear") == -1):
                        self.setOrder(int(mapNumber)*1000+995)
                        self.setDirectory(pageName)
                        self.setName("Chapter " + mapNumber + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"] + " [ Ending ]")
                    else:
                        self.setOrder(int(mapNumber)*1000+990)
                        self.setDirectory(pageName)
                        self.setName("Chapter " + mapNumber + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"])
                elif not (pageName.find("open") == -1):
                    self.setOrder(int(mapNumber)*1000)
                    self.setDirectory(pageName)
                    self.setName("Chapter " + mapNumber + " (Japanese): " + mapName[mapNumber]["name"] + " [ Opening ]")
                elif not (pageName.find("ending") == -1):
                    self.setOrder(int(mapNumber)*1000+980)
                    self.setDirectory(pageName)
                    self.setName("Chapter " + mapNumber + " (Japanese): " + mapName[mapNumber]["name"] + " [ Ending ]")
                else:
                    # ===============
                    # Multiple scenes
                    # ===============
                    if len(pageName.split("-")) > 2:
                        ...
                        self.setDirectory(pageName)
                        self.setName("Chapter " + mapNumber + "-" + dungeonNumber + " (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["dungeon"][dungeonNumber] + " Scene " + sceneNumber)
                    # ===============
                    # Single scene
                    # ===============
                    else:                        
                        ...
                        self.setDirectory(pageName)
                        self.setName("Chapter " + mapNumber + "-" + dungeonNumber + " (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["dungeon"][dungeonNumber])
            else:
                ...
                if (str(mapNumber) in [str(i) for i in range(100, 200)]):
                    if not (pageName.find("dungeon_ex") == -1):
                        if not (pageName.find("ex1_open") == -1):
                            ...
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"] + " [ Opening 1 ]")
                        elif not (pageName.find("ex1_clear") == -1):
                            self.setOrder(int(mapNumber)*1000+992)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"] + " [ Ending 1 ]")
                        elif not (pageName.find("ex2_open") == -1):
                            self.setOrder(int(mapNumber)*1000+993)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"] + " [ Opening 2 ]")
                        elif not (pageName.find("ex2_clear") == -1):
                            self.setOrder(int(mapNumber)*1000+994)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"] + " [ Ending 2 ]")
                        elif not (pageName.find("ex_clear") == -1):
                            self.setOrder(int(mapNumber)*1000+995)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"] + " [ Ending ]")
                        else:
                            self.setOrder(int(mapNumber)*1000+990)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " Special (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["special"])
                    elif not (pageName.find("open") == -1):
                        if not (pageName.find("open1") == -1):
                            self.setOrder(int(mapNumber)*1000)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " (Japanese): " + mapName[mapNumber]["name"] + " [ Opening 1 ]")
                        elif not (pageName.find("open2") == -1):
                            self.setOrder(int(mapNumber)*1000 + 1)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " (Japanese): " + mapName[mapNumber]["name"] + " [ Opening 2 ]")
                        else:
                            self.setOrder(int(mapNumber)*1000)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " (Japanese): " + mapName[mapNumber]["name"] + " [ Opening ]")
                    elif not (pageName.find("ending") == -1):
                        self.setOrder(int(mapNumber)*1000+980)
                        self.setDirectory(pageName)
                        self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + " (Japanese): " + mapName[mapNumber]["name"] + " [ Ending ]")
                    else:
                        # ===============
                        # Multiple scenes
                        # ===============
                        if len(pageName.split("-")) > 2:
                            ...
                            self.setOrder(int(mapNumber)*1000+int(dungeonNumber)*10+int(sceneNumber))
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + "-" + dungeonNumber + " (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["dungeon"][dungeonNumber] + " Scene " + sceneNumber)
                        # ===============
                        # Single scene
                        # ===============
                        else:
                            ...
                            self.setOrder(int(mapNumber)*1000+int(dungeonNumber)*10)
                            self.setDirectory(pageName)
                            self.setName("Summoner's Arc Chapter " + str(int(mapNumber) % 100) + "-" + dungeonNumber + " (Japanese): " + mapName[mapNumber]["name"] + " - " + mapName[mapNumber]["dungeon"][dungeonNumber])                        
                else:
            ...
            if pageName == "grand_op":
                ...
                self.setDirectory(pageName)
                self.setName("Grand Quest Introduction - Dawn of the Summoner (Japanese)")
            elif pageName == "grand_op2":
                self.setOrder(300001)
                self.setDirectory(pageName)
                self.setName("Grand Quest Introduction - Saga of Heroes (Japanese)")
            else:
                grandQuestNumber = pageName.split("_")[1]
                if not (pageName[9:].find("op") == -1):
                    self.setOrder(300000 + int(grandQuestNumber)*1000)
                    self.setDirectory(pageName)
                    self.setName("Grand Quest " + grandQuestNumber + ": " + grandQuestName[str(int(grandQuestNumber))]["name"] + " -- Opening Scene (Japanese)")
                else:
                    grandQuestSceneNumber = pageName[9:]
                    self.setOrder(300000 + int(grandQuestNumber)*1000 + int(grandQuestSceneNumber))
                    self.setDirectory(pageName)
                    self.setName("Grand Quest " + grandQuestNumber + ": " + grandQuestName[str(int(grandQuestNumber))]["name"] + " -- Scene " + pageName[9:] + " (Japanese)")
